# pi-compaction-i18n — Agent 规范

## 项目一句话定位
Pi 扩展：让 pi 的 `/compact` 和 `/tree` 分支摘要支持 11 种语言本地化，从系统 locale 自动检测。npm 包 `pi-compaction-i18n`。

## 先读文档顺序
1. 本文件
2. `README.md`（功能矩阵、支持语言表、locale 检测优先级）
...
## 验证方式
`pi install` 后 `/compact` `/tree` 看输出语言；`/reload`
